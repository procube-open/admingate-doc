...
1. ステップ 1: 信頼されたエンティティを選択 
   ...
   1. 許可されるアクセスで「プログラムと AWS マネジメントコンソールへのアクセスを許可する」を選択する
...
1. ステップ 3: 名前、確認、および作成
   1. ロールの詳細
      1. ロール名に任意のわかりやすい名前を入力する。「IDManager の再構築」の章で設定する[環境変数名 aws_role_switchonly](../hive.md#環境変数の設定) の値と同名にする（例: AG_SwitchOnlyRole）ため、ロール名を控えておく
      1. 説明を入力する（例: switch only role for AdminGate）
   1. "ステップ 1: 信頼されたエンティティを選択する" の内容が「3. ステップ 1: 信頼されたエンティティを選択」で選択した内容であることを確認する（ただし、後で Action に追加設定を行う）
...
1. 信頼関係タブ - 信頼されたエンティティで「信頼ポリシーを編集」をクリックする
1. 信頼ポリシーの action に `sts:TagSession` を追加するよう JSON 形式で編集する。（内容例: 後述の [信頼関係タブ - 信頼されたエンティティ（例：切り替え専用）](#信頼関係タブ---信頼されたエンティティ例切り替え専用)）


更に、「切り替え専用のポリシー」を作成します。<!--ただし、ロール作成時に準備済みの切り替え専用のポリシー（カスタマー管理ポリシー）を設定済みの場合は、この手順は必要ありません。-->
...
- Resource の ``role/*`` は更に絞り込んでもよい（例：``role/AGM_*``）
:::
...
- SAML:aud のサインインのエンドポイント URLの `SAMLSP(省略)` は作成した ID プロバイダのインスタンスID。[切り替え専用ロールを作成する](#切り替え専用ロールを作成する)の「3.-vi. サインインのエンドポイント URL」で控えた各エンドポイントの属性値にする。ID プロバイダの詳細を開いて「サインイン URL」で確認することも可能
- SAML:aud のサインインのエンドポイント URLの `/acs/SAMLSP(省略)` 部分は省略可能。省略した場合は、Keycloak の設定（Valid redirect URIs、および Assertion Consumer Service POST Binding URL）も合わせる
:::
...
- サインインのエンドポイント URL
   ...
   以下のいずれかの方法で取得する
   1. [切り替え専用ロールを作成する](#切り替え専用ロールを作成する) で控えた「3.-vi. サインインのエンドポイント URL」
