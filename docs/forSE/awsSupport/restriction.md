...
:::tip
指定できる作業者（単体選択）／作業者（チーム選択）の上限数の目安

ユーザIDおよびチームIDが64文字で、ロールの信頼ポリシーの長さ制限値が2048文字の場合：
- 作業者（単体選択）のみ指定のとき　… 23人
- 作業者（チーム選択）のみ指定のとき　… 21チーム
- 作業者（単体選択）と作業者（チーム選択）の両方指定のとき　… 8人＋8チーム（比率半々の場合）

ロールの信頼ポリシーの長さ制限値は、引き上げが可能。[AWS 公式ドキュメント - IAM オブジェクトクォータ](https://docs.aws.amazon.com/ja_jp/IAM/latest/UserGuide/reference_iam-quotas.html#autoapproved)
:::
...
:::tip
- ロールの登録中にエラーが発生したためロールバックを行ったが、ネットワークエラー等でロールバックも失敗した後に、ロールが残ったままプロビジョニングタスクを「再試行」すると当現象が発生する可能性があります。ロールバックに失敗した場合は、プロビジョニングタスクに後述(5)のエラーが出力されていることがあります。
...
```
#### 対応：
- 時間をおいて（ネットワークエラー等が解決してから）、プロビジョニングタスクを「再試行」する


### (5) ロールバックに失敗したとき
#### エラー：
```
Failed to attach policies, and failed to rollback. [Caution] The role may remain. Exception detail(attachPolicy): {error}, Exception detail(rollback): {rollback_error}
```
※{error} および {rollback_error} には詳細エラーが表示されます。

#### 対応：
- エラーメッセージをシステム管理者にご連絡ください

:::tip
- 作業申請で指定した IAM ポリシーが AWS IAM ポリシーに存在しないときや、ポリシーをアタッチする権限がないときなど、ロールの登録に失敗するため、ロールバックが実行されます。しかし、ロールバック中にネットワークエラーなどで再試行の上限を超えてもエラーが解決せず、ロールバックに失敗したとき、ロールが残る可能性があります。
- AWS IAM コンソールで該当ロールを確認し、IDManager からの登録ロールか、その他での登録かでそのロールを削除するか保持するかを判断してください。
- 必要に応じて元のエラー（例：IAM ポリシーがない）を解決し、プロビジョニングタスクを「再試行」、または新しい作業申請で申請・承認しなおしてください。
:::
